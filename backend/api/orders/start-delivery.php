...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $stmt = $pdo->prepare("
            ...
            WHERE id = ? AND delivery_person_id = ? AND status = 'assigned'
        ");
        $stmt->execute([$orderId, $deliveryPersonId]);
        $order = $stmt->fetch(PDO::FETCH_ASSOC);
        
        if (!$order) {
            sendError('Commande non trouvée ou non assignée', 404);
        }
        
        // Mettre à jour le statut en "preparation"
        ...
        if ($result) {
            // Notifier le client
            $this->notifyCustomer($orderId, 'Votre commande est en préparation');
            ...
            sendError('Erreur lors du démarrage de la livraison', 500);
        }
    } else {
    ...
    error_log("Erreur PDO start-delivery.php: " . $e->getMessage());
    sendError('Erreur de base de données', 500);
} catch (Exception $e) {
    error_log("Erreur start-delivery.php: " . $e->getMessage());
...
function notifyCustomer($orderId, $message) {
    ...
    try {
        // Récupérer les infos de la commande pour notifier
        $stmt = $pdo->prepare("
        ...
        if ($order && $order['customer_email']) {
            ...
            error_log("Notification client pour commande $orderId: " . $message);
        }
    } catch (Exception $e) {
